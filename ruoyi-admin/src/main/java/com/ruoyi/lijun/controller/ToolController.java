//package com.ruoyi.yzyx.controller;
//
//import com.alibaba.fastjson.JSON;
//import com.ruoyi.common.config.Global;
//import com.ruoyi.common.core.domain.AjaxResult;
//import com.ruoyi.common.utils.file.FileUploadUtils;
//import com.ruoyi.yzyx.domain.YzyxMedia;
//import com.ruoyi.yzyx.service.IYzyxMediaService;
...
//    private Dao dao;
//    @Autowired
//    private IYzyxMediaService mediaService;
//    @Autowired
...
//        try {
//            List<YzyxMedia>medias=new ArrayList<>();
//            if (files!=null&&files.length>0) {
//                Long maxBaseSort=mediaService.selectMaxSortByBaseId(Tool.IFNULL(base_id,"随便写的值"));
//                for (int i=0,len=files.length;i<len;i++) {
//                    if (files[i]!=null&&!files[i].isEmpty()) {
//                        if(files[i].getContentType().startsWith("image")&&!WXUtils.checkPic(files[i],cacheManager.getCache(FSS.CONSTANT).get(FSS.XCX_TOKEN).toString()))return AjaxResult.error("图片内容违规");
//                        String fileName= FileUploadUtils.upload(Global.getUploadPath(), files[i]);
////                        String url = serverConfig.getUrl() + fileName;//带域名的绝对路径,留着备用
//                        YzyxMedia media=new YzyxMedia();
...
//                            mediaService.insertYzyxMedia(media);
//                        }
//                        medias.add(media);
//                    }
//                }
...
//    @ResponseBody
//    public AjaxResult uploadMediaByBaseIdToAli(String base_id, Integer sort, MultipartFile... files){
//        try {
//            List<YzyxMedia>medias=new ArrayList<>();
//            if (files!=null&&files.length>0) {
//                Long maxBaseSort=mediaService.selectMaxSortByBaseId(Tool.IFNULL(base_id,"随便写的值"));
...
//                        }
//                        Map<String,Object>result=new OSSClientUtil(configService.selectConfigByKey("ali.access_key_id"),configService.selectConfigByKey("ali.access_key_secret"),configService.selectConfigByKey("ali.bucket_name")).uploadFile2OSS(files[i]);
...
//    @ResponseBody
//    AjaxResult updateMedia(Long mediaId, String baseId, Long sort, MultipartFile files){
//        if(Tool.isNull(mediaId))return AjaxResult.error("缺少ID");
//        try {
//            if(files.getContentType().startsWith("image")&&!WXUtils.checkPic(files,cacheManager.getCache(FSS.CONSTANT).get(FSS.XCX_TOKEN).toString()))return AjaxResult.error("图片内容违规");
//            YzyxMedia YzyxMedia=mediaService.selectYzyxMediaById(mediaId);
//            BufferedImage bufferedImage=ImageIO.read(files.getInputStream());
//            if(bufferedImage!=null){
//                YzyxMedia.setWidth(bufferedImage.getWidth());
//                YzyxMedia.setHeight(bufferedImage.getHeight());
//            }
//            if(YzyxMedia==null)return AjaxResult.error("要更新的数据不存在,ID:"+mediaId);
//            if(!Tool.isNull(baseId))YzyxMedia.setBaseId(baseId);
//            if(!Tool.isNull(sort))YzyxMedia.setBaseSort(sort);
//            Map<String,Object>result=new OSSClientUtil(configService.selectConfigByKey("ali.access_key_id"),configService.selectConfigByKey("ali.access_key_secret"),configService.selectConfigByKey("ali.bucket_name")).uploadFile2OSS(files);
//            if(!Tool.isNull(YzyxMedia.getOtherId()))new OSSClientUtil(configService.selectConfigByKey("ali.access_key_id"),configService.selectConfigByKey("ali.access_key_secret"),configService.selectConfigByKey("ali.bucket_name")).deleteFile2OSS(new ArrayList<>(Arrays.asList(String.valueOf(YzyxMedia.getOtherId()))));
//            YzyxMedia.setUrl(result.get("url").toString());
//            YzyxMedia.setOtherId(result.get("objectName").toString());
//            if(mediaService.updateYzyxMedia(YzyxMedia)>0)return AjaxResult.success("更新成功",YzyxMedia);
...
//    @ResponseBody
//    AjaxResult updateBannerImg(Long mediaId, String baseId, Long sort, MultipartFile files){
//        if(Tool.isNull(mediaId))return AjaxResult.error("缺少ID");
//        try {
//            if(files.getContentType().startsWith("image")&&!WXUtils.checkPic(files,cacheManager.getCache(FSS.CONSTANT).get(FSS.XCX_TOKEN).toString()))return AjaxResult.error("图片内容违规");
//            YzyxMedia YzyxMedia=mediaService.selectYzyxMediaById(mediaId);
...
//        int deleteResult=mediaService.deleteYzyxMediaByIds(ids);
...
//            HttpPost httpPost = new HttpPost("https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token="+cacheManager.getCache(FSS.CONSTANT).get(FSS.XCX_TOKEN));
...
//        if(!WXUtils.checkPic(files,cacheManager.getCache(FSS.CONSTANT).get(FSS.XCX_TOKEN).toString()))return AjaxResult.error("图片内容违规");
